...
export default function OnboardingLanguagesPage() {
  ...
  const [learning, setLearning] = React.useState<Record<string, string>>({
    JA: "Intermediate",
  })
    ...
    <div>
      ...
      <div className="mt-8 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {languageOptions
              .filter((l) => !native.includes(l.code))
              .map((lang) => {
                ...
                return (
                  ...
                  >
                    ...
                    <select
                      value={level}
                      onChange={(e) =>
                        setLearning((prev) => ({ ...prev, [code]: e.target.value }))
                      }
      ...
      <div className="mt-8 flex justify-between">
        <Button variant="ghost" render={<Link href="/ai-preferences" />}>Back</Button>
        <Button render={<Link href="/interests" />}>Continue</Button>
      </div>
    </div>
  )
}
