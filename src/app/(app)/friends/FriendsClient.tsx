...
import type { FriendCard, FriendLists } from "@/lib/friends"
...
function LanguageBadges({ user }: { user: FriendCard }) {
  return (
...
function UserAvatar({ user }: { user: FriendCard }) {
...
/*
 * Only the interactive half of the page. The lists arrive from the server
 * already rendered, so this holds them as state purely so an accept, decline or
 * removal can update the tabs immediately rather than waiting on a refresh.
 */
export function FriendsClient({ initial }: { initial: FriendLists }) {
  const router = useRouter()
  const [data, setData] = React.useState<FriendLists>(initial)
  const [lastFromServer, setLastFromServer] = React.useState<FriendLists>(initial)
  const [busy, setBusy] = React.useState<Record<string, boolean>>({})

  // The server is the source of truth: once router.refresh() delivers new lists,
  // adopt them instead of keeping the optimistic copy forever. Adjusted during
  // render rather than in an effect, which would render twice and re-introduce
  // the cascading updates this app has already had to remove once.
  ...
  async function acceptRequest(user: FriendCard) {
  ...
  async function openChat(user: FriendCard) {
    setBusyFor(user.id, true)
    try {
