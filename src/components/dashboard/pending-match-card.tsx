 ...
 *
 * Extended to voice (roadmap #40, 18.5) alongside chat — video has no
 * availability path (a call needs both sides live for the call itself), so
 * `match.type` here is only ever `"chat"` or `"voice"` in practice.
 */
export function PendingMatchCard({ matches }: { matches: MatchResult[] }) {
  ...
  return (
    <div className="space-y-3">
      {matches.map((match) => {
        const isVoice = match.type === "voice"
        return (
          <article
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {match.compatibilityPct}% match · {isVoice ? "voice conversation" : "text conversation"}
                </p>
              </div>
            </div>
            <Button
              ...
              render={
                <Link
                  href={isVoice ? `/session/voice/${match.conversationId}` : `/messages/${match.conversationId}`}
                />
              }
            >
              {isVoice ? <Mic className="size-4" /> : <MessageSquare className="size-4" />}{" "}
              {isVoice ? "Join voice call" : "Open conversation"} <ArrowRight className="size-4" />
            </Button>
          </article>
        )
      })}
